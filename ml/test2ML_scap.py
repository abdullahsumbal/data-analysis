...
#    Get    the    data
df = pd.read_csv("test2ML_scap.csv")
...
reg_scap = Pipeline([('scl',    StandardScaler()), ('clf',    svm.SVR(kernel='rbf', gamma=0.85))])
#    Fit    the    variables    to    the    PCE
reg_scap.fit(variables, df.scap)
#    Get    the    predicted    Specific Capacity from    the    model    and    save    it    to    the    DataFrame
df['scap_pred_svm']    = reg_scap.predict(variables)
...
fig, ax1    = plt.subplots(1,    1,    clear=True, num='scap_pred', figsize=(5,    4))
ax1.set_title('Prediction vs Experimental')

for label,    data    in df.groupby('label'):
                plt.plot('scap',    'scap_pred_svm',    'o',    color=data['color'].iloc[0],
                                                    data=data,    label=label)
plt.legend()
plt.autoscale(enable=True)
plt.plot([0, 200],    [0, 200],    ls="--",    c=".3")
ax1.set_ylabel('Predicted Specific Capacity (mAh/g)')
ax1.set_xlabel('Measured Specific Capacity (mAh/g)')
plt.tight_layout()
...
c    = reg_scap.predict(r).reshape(x_len, y_len)
#    Make    a    contour    map

cmap  = ax.contour(xs,    ys,    c,    vmin=0,    vmax=7,    cmap='gray_r')
plt.clabel(cmap,    inline=1,    fontsize=10)
#    Make    a    value    map
pmap    = ax.pcolormesh(xs,    ys,    c,    shading='gouraud',  vmin=20,    vmax=40,    cmap='viridis')
